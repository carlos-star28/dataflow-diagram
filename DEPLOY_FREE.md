...
```bash
docker exec dataflow-db sh -c 'mysqldump -uroot -p"$MYSQL_ROOT_PASSWORD" dataflow_digram' > backup.sql
```

### Restore MySQL

```bash
cat backup.sql | docker exec -i dataflow-db sh -c 'mysql -uroot -p"$MYSQL_ROOT_PASSWORD" dataflow_digram'
```
